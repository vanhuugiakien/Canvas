...
export class PaintComponent implements OnInit {
  constructor(public socket: SocketService, public fs: AngularFirestore) {}
  ...
  ngOnInit(): void {
    ...
    this.socket.socket.on('draw', async (data) => {
      let a;
      if (data) {
        await this.fs
          .collection('canvas')
      ...
      function (mouseEvent) {
        ...
        switch (drawer.selectedShape) {
          ...
          case drawer.availableShapes.CIRCLE:
            drawer.selectedElement = new Circle(
              pos,
              drawer.currentSettings(),
              0
            );
          ...
          case drawer.availableShapes.LINE:
            drawer.selectedElement = new Line(
              pos,
              drawer.currentSettings(),
              pos
            );
          ...
          case drawer.availableShapes.LINE_LIST:
            drawer.selectedElement = new LineList(
              pos,
              drawer.currentSettings()
            );
            break;
          case drawer.availableShapes.DrawnText:
            ...
            drawer.selectedElement = new DrawnText(
              pos,
              drawer.currentSettings()
            );
            break;
    ...
    // endregion
    // this.socket.socket.on('draw', (data) => {
    //   let lst = [];
    //   for (let i = 0; i < data.shapes.length; i++) {
    //     let tmp = JSON.parse(JSON.stringify(data.shapes[i]));
    ...
    let a =  () => {
      ...
      this.socket.Draw(drawer.shapes);
    ...
    document.querySelectorAll('#shape-list li').forEach(
       ...
       */
      function (elem: HTMLCanvasElement) {
    ...
    widthDecrease.addEventListener(
       ...
       */
      function (evt) {
        widthValue.innerHTML = Math.max(
          1,
          parseInt(widthValue.innerHTML) - 1
        ).toString();
      }
    );
    ...
    document
      .getElementById('img-save')
      .addEventListener('click', saveAsJsonFile);
    // endregion
